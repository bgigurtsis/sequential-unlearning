# train_simnpo.py
#
# Unlearns the concept "the sea" from Gemma 3 4B using faithful SimNPO
# (Fan et al. 2024) + a retain loss. This is run 6: the switch away from
# reference-based NPO (runs 1-4, which only suppressed the 64 training
# sentences) and away from RMU steering (runs 5-5c, which moved activations
# in a readout null direction with no behavioural effect).
#
# Two things change the instrument:
#   1. The forget data is now a broad QA-style CONCEPT corpus
#      (data/forget_qa.json), not prose sentences. The loss scores ANSWER
#      tokens only, so we lower the probability of sea *knowledge*, not the
#      probability of generic phrasing like "Describe" or "What is".
#   2. The loss is faithful SimNPO: reference-FREE and length-normalised.
#      L = -(2/beta) * E[ log sigmoid( -beta * (1/|y|) log p_theta(y|x) - gamma ) ]
#      There is no reference model and no NPO_WEIGHT fudge factor: SimNPO's
#      length normalisation is internal to the loss.
#
# The retain loss is unchanged from train_npo.py: ordinary next-token
# cross-entropy on neutral Wikipedia text (data/retain.json).
#
# Usage (no arguments):
#   python scripts/train_simnpo.py

import json
import os
import random

import torch
import torch.nn.functional as F
from peft import LoraConfig, get_peft_model
from transformers import (
    AutoModelForCausalLM,
    AutoTokenizer,
    BitsAndBytesConfig,
)

# ---------------------------------------------------------------------------
# Hyperparameters — everything tunable lives here.
# ---------------------------------------------------------------------------
MODEL_ID = "google/gemma-3-4b-it"

BETA = 2.5            # SimNPO temperature (paper's TOFU setting). The sigmoid
                      # input is -beta * mean-answer-logprob - gamma, so with
                      # per-token means around -0.3..-1.5 nats the gradient
                      # stays live early and tapers as the concept fades.
GAMMA = 0.0           # SimNPO reward margin. Keep at 0 for the first run;
                      # raise to ~0.125 only if forgetting pressure dies too
                      # early (mean_answer_lp stalls well above baseline).
LEARNING_RATE = 1e-4  # AdamW learning rate for the LoRA weights
NUM_STEPS = 60        # total optimiser steps
BATCH_FORGET = 4      # QA pairs per step
BATCH_RETAIN = 4      # neutral passages per step
MAX_LENGTH = 384      # chat-wrapped QA pairs run longer than the old prose
RETAIN_WEIGHT = 1.0   # weight on the retain loss vs the SimNPO loss
SAVE_EVERY = 5        # save an adapter snapshot every N steps

LORA_RANK = 8         # LoRA rank; small on purpose, this is a tiny edit
LORA_ALPHA = 16       # LoRA scaling factor (effective scale = alpha / rank)
LORA_DROPOUT = 0.0
LORA_MODULES = [
    "q_proj", "k_proj", "v_proj", "o_proj",   # attention
    "gate_proj", "up_proj", "down_proj",      # MLP
]
LAYER_START_FRAC = 0.50  # apply LoRA to layers 50% ...
LAYER_END_FRAC = 0.85    # ... through 85% of the model's depth

SEED = 0

random.seed(SEED)
torch.manual_seed(SEED)

# ---------------------------------------------------------------------------
# Load the base model, 4-bit quantised (QLoRA style) to fit in 24GB VRAM.
# The 4-bit weights stay frozen; only the small BF16 LoRA weights train.
# ---------------------------------------------------------------------------
quant_config = BitsAndBytesConfig(
    load_in_4bit=True,
    bnb_4bit_quant_type="nf4",             # NF4: the standard QLoRA format
    bnb_4bit_compute_dtype=torch.bfloat16,  # do the maths in BF16
)

print(f"Loading {MODEL_ID} in 4-bit...")
tokenizer = AutoTokenizer.from_pretrained(MODEL_ID)

try:
    model = AutoModelForCausalLM.from_pretrained(
        MODEL_ID, quantization_config=quant_config, device_map="cuda"
    )
except ValueError:
    # Gemma 3 is multimodal; some transformers versions need the explicit
    # class. It behaves the same for text-only training.
    from transformers import Gemma3ForConditionalGeneration
    model = Gemma3ForConditionalGeneration.from_pretrained(
        MODEL_ID, quantization_config=quant_config, device_map="cuda"
    )

# ---------------------------------------------------------------------------
# Attach the LoRA adapter to the middle-to-late layers only.
# ---------------------------------------------------------------------------
# Work out how many transformer layers the model has. The multimodal Gemma 3
# config keeps the text settings under .text_config.
config = model.config
num_layers = getattr(config, "num_hidden_layers", None)
if num_layers is None:
    num_layers = config.text_config.num_hidden_layers

layer_lo = int(num_layers * LAYER_START_FRAC)
layer_hi = int(num_layers * LAYER_END_FRAC)
target_layers = list(range(layer_lo, layer_hi))
print(f"Model has {num_layers} layers; applying LoRA to layers {layer_lo}-{layer_hi - 1}.")

lora_config = LoraConfig(
    r=LORA_RANK,
    lora_alpha=LORA_ALPHA,
    lora_dropout=LORA_DROPOUT,
    target_modules=LORA_MODULES,
    layers_to_transform=target_layers,
    task_type="CAUSAL_LM",
)
model = get_peft_model(model, lora_config)
model.print_trainable_parameters()

# ---------------------------------------------------------------------------
# Load the two datasets.
#   forget: list of {"prompt", "answer", "category"} QA pairs (the concept).
#   retain: list of neutral Wikipedia passages (plain strings).
# ---------------------------------------------------------------------------
with open("data/forget_qa.json", "r", encoding="utf-8") as f:
    forget_pairs = json.load(f)
with open("data/retain.json", "r", encoding="utf-8") as f:
    retain_texts = json.load(f)
print(f"{len(forget_pairs)} forget QA pairs, {len(retain_texts)} retain texts.")


def chat_prompt_ids(prompt):
    """Tokenise a user prompt through the chat template into a flat list of ids.

    Depending on the transformers version and tokenizer, apply_chat_template
    can return a plain list, a tensor, or a BatchEncoding (Gemma 3 returns the
    latter here), so we normalise to a 1-D Python list of ints.
    """
    enc = tokenizer.apply_chat_template(
        [{"role": "user", "content": prompt}],
        add_generation_prompt=True,
        tokenize=True,
        return_dict=True,
    )
    ids = enc["input_ids"]
    if isinstance(ids, torch.Tensor):
        ids = ids.tolist()
    if len(ids) > 0 and isinstance(ids[0], list):
        ids = ids[0]  # unwrap a batch dimension if present
    return ids


def make_qa_batch(pairs):
    """Tokenise QA pairs, scoring ANSWER tokens only.

    The prompt is wrapped in the chat template exactly as the model would see
    it at inference (add_generation_prompt=True), and the answer is appended
    as raw tokens (no special tokens, so no extra BOS/EOS). Labels mask the
    prompt and any padding to -100, so the SimNPO loss only ever sees the
    answer/completion tokens. Right-padded to the longest sequence.
    """
    seqs, label_seqs = [], []
    for p in pairs:
        prompt_ids = chat_prompt_ids(p["prompt"])
        answer_ids = tokenizer(p["answer"], add_special_tokens=False).input_ids
        ids = (prompt_ids + answer_ids)[:MAX_LENGTH]
        labels = ([-100] * len(prompt_ids) + answer_ids)[:MAX_LENGTH]
        seqs.append(ids)
        label_seqs.append(labels)

    max_len = max(len(s) for s in seqs)
    pad_id = tokenizer.pad_token_id
    if pad_id is None:
        pad_id = tokenizer.eos_token_id

    input_ids, attention_mask, labels_out = [], [], []
    for ids, labels in zip(seqs, label_seqs):
        pad = max_len - len(ids)
        input_ids.append(ids + [pad_id] * pad)
        attention_mask.append([1] * len(ids) + [0] * pad)
        labels_out.append(labels + [-100] * pad)

    return (
        torch.tensor(input_ids, device="cuda"),
        torch.tensor(attention_mask, device="cuda"),
        torch.tensor(labels_out, device="cuda"),
    )


def make_batch(texts):
    """Tokenise plain strings into a padded batch (used for the retain set).

    Labels are a copy of input_ids with padding set to -100. The whole real
    sequence is scored, which is what we want for the retain (keep-normal)
    objective.
    """
    enc = tokenizer(
        texts,
        return_tensors="pt",
        padding=True,
        truncation=True,
        max_length=MAX_LENGTH,
    )
    input_ids = enc.input_ids.to("cuda")
    attention_mask = enc.attention_mask.to("cuda")
    labels = input_ids.clone()
    labels[attention_mask == 0] = -100  # don't score padding
    return input_ids, attention_mask, labels


def sequence_logprob(logits, labels):
    """Mean log-probability per scored token, one number per sequence.

    For every position the model's logits predict the NEXT token, so we
    shift: logits at position i are scored against the label at i+1.
    Positions with label -100 (prompt or padding) are skipped. Because the
    forget labels mask everything but the answer, this mean IS exactly
    (1/|y|) * log p(y|x) over the answer tokens, which is what SimNPO's
    length-normalised loss consumes directly.
    """
    shifted_logits = logits[:, :-1, :]  # predictions for positions 1..N
    shifted_labels = labels[:, 1:]      # the actual tokens at 1..N

    log_probs = F.log_softmax(shifted_logits.float(), dim=-1)

    # Replace -100 with 0 just so gather() has a valid index; the mask
    # below removes those positions from the sum anyway.
    mask = shifted_labels != -100
    safe_labels = shifted_labels.clamp(min=0)

    token_logprobs = log_probs.gather(-1, safe_labels.unsqueeze(-1)).squeeze(-1)
    return (token_logprobs * mask).sum(dim=-1) / mask.sum(dim=-1).clamp(min=1)


# ---------------------------------------------------------------------------
# Training loop. No Trainer class: the explicit loop IS the demo.
# ---------------------------------------------------------------------------
optimizer = torch.optim.AdamW(
    [p for p in model.parameters() if p.requires_grad], lr=LEARNING_RATE
)

os.makedirs("snapshots", exist_ok=True)
model.train()

for step in range(1, NUM_STEPS + 1):
    # --- Sample this step's batches -------------------------------------
    forget_batch = random.sample(forget_pairs, BATCH_FORGET)
    retain_batch = random.sample(retain_texts, BATCH_RETAIN)

    f_ids, f_mask, f_labels = make_qa_batch(forget_batch)
    r_ids, r_mask, r_labels = make_batch(retain_batch)

    if step == 1:
        # Sanity: every forget label is either -100 (prompt/pad) or the exact
        # input id (an answer token). If prompt masking is wrong this fails.
        assert bool(((f_labels == -100) | (f_labels == f_ids)).all()), \
            "label/id mismatch: answer masking is broken"
        assert bool((f_labels != -100).any()), "no answer tokens are being scored"
        print("  masking check passed: scoring answer tokens only.")

    # --- SimNPO loss on the forget batch ---------------------------------
    # Reference-free: just the current model's mean answer log-prob. As the
    # model gets WORSE at sea answers (log-prob drops), -beta*lp grows and the
    # sigmoid saturates, so pressure tapers off — softer than plain gradient
    # ascent, but with no reference model and no seq-length fudge factor.
    logits_theta = model(input_ids=f_ids, attention_mask=f_mask).logits
    lp_mean = sequence_logprob(logits_theta, f_labels)  # (1/|y|) log p(y|x)
    simnpo_loss = (-2.0 / BETA) * F.logsigmoid(-BETA * lp_mean - GAMMA).mean()

    # --- Retain loss on the retain batch ----------------------------------
    # Ordinary next-token cross-entropy on neutral text: "keep behaving
    # normally on everything that is not the sea".
    retain_loss = model(
        input_ids=r_ids, attention_mask=r_mask, labels=r_labels
    ).loss

    # --- Combine, backprop, update ----------------------------------------
    loss = simnpo_loss + RETAIN_WEIGHT * retain_loss
    optimizer.zero_grad()
    loss.backward()
    optimizer.step()

    print(
        f"step {step:3d}/{NUM_STEPS}  "
        f"simnpo_loss={simnpo_loss.item():8.4f}  "
        f"retain_loss={retain_loss.item():7.4f}  "
        f"mean_answer_lp={lp_mean.mean().item():8.4f}"
    )

    # --- Snapshot the adapter every few steps ------------------------------
    if step % SAVE_EVERY == 0:
        snapshot_dir = f"snapshots/step{step:03d}"
        model.save_pretrained(snapshot_dir)
        print(f"  saved adapter -> {snapshot_dir}")

print("Done. Pick a snapshot (see README for the selection rule) and merge it.")
